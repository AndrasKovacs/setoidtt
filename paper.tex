...
polymorphic types, mainly in relation to GHC and ML-like languages \cite{leijen2008hmf,leijen2009flexible,serrano2018guarded,vytiniotis2006boxy,serrano2020a}.
...
later we will leave them implicit.
\begin{alignat*}{3}
...
computed to iterated implicit function types when the domain telescope is known
to be empty or non-empty. See \LabTirName{fun-$\epsilon$} and
\LabTirName{fun-$\TCons$}: a curried function with empty domain computes to
simply the codomain, while a function with a non-empty domain computes to an
implicit function type. We explicitly notate telescopes in both
$\lambda$-abstractions and applications for strictly curried functions, since
they are relevant in the computation rules.
...
\end{enumerate}
In the following, we shall only handle the case for $\lambda$ insertions. This
allows a simple implementation which only requires minimal changes to
unification, and which is already remarkably powerful. It seems that handling
implicit application insertions requires extending unification; we will talk
about this in Section \ref{TODO}. But before delving into elaboration, we need to
rectify a curious issue.

\subsection{Superfluous Implicit Functions}

Assuming $Bool : \U$, $true : Bool$ and $false : Bool$, consider the following surface expression:
\[
  \slet\,x : \_ = true\,\sin\,x
\]
...
to use $x$. To rule out such nonsense, we adopt the following principle:
...
the surface input. With $\slet\,x : \_ = true$, the old elaborator checks $true$
with a fresh meta, and just assumes that the meta does not stand for an
implicit function type.

To implement our principle, we need \emph{constancy constraints}. These are
constraints in the sense of GHC's OutsideIn(X) \cite{vytiniotis2011outsidein}
and also in the sense of the implementation in Agda and Coq. Rules for constancy
constraints are as follows.

\begin{mathpar}
  \inferrule*[lab=metacon/constr]
             {\Theta \vdash \alpha : \Gamma \To \Tel \\ \Theta|\Delta,\,x : \Rec\,(\alpha\,\ol{t}) \vdash B : \U}
             {\Theta,\,\mathsf{constancy}_{\Delta,\,x : \Rec\,(\alpha\,\ol{t})}\,B\vdash}

  \inferrule*[lab=constr/solve]
             {\Theta_0 \vdash \alpha : \Gamma_0 \To \Tel \\ x \notin FreeVars(B)}
             {\Theta_0,\,\mathsf{constancy}_{\Delta,\,x : \Rec\,(\alpha\,\ol{t})}\,B,\,\Theta_1|\Gamma_1
               \vdash \alpha \equiv \lambda\,\overline{\mathsf{vars}_{\Gamma_0}}.\,\epsilon}
\end{mathpar}

In the rule \LabTirName{metacon/constr}, we allow extension of the metacontext
with a constraint.  In the constraint, the metavariable $\alpha : \Gamma \To
\Tel$ represents an unknown domain for a curried function type, and $B$ is the
codomain of the function. Here, $\Gamma \To \Tel$ refers to a closing type, which
was defined in Section \ref{def:closingtype}.

\todo{first, define new closing types and fresh metas!}

The rule \LabTirName{constr/solve} says that if $B$ does not depend on the $x :
\Rec\,(\alpha\,\ol{t})$ domain variable, then $\alpha$ is equal to the constant
function which returns the empty telescope. We use the abbreviation
