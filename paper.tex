...
about 1200 lines of Haskell. Of this, 673 lines constitute the core syntax,
evaluation, unification and elaboration. Of this 673 lines, 458 lines implement
...
prior solutions can scale to dependent types. In particular, QL's preprocessing
step is not applicable, since in a dependently typed spine we cannot do anything
with later arguments until previous arguments are actually elaborated, or else
it has to be restricted to non-dependent spines.

Secondly, FCIF supports global inference and does not handle neutral spines
specially. For an example for global inference, $\slet\,x : \_ =
single\,id\,\sin\,$ $cons\,\{IdTy\}\,x\,nil$ works in FCIF, MLF and HMF but does
not work in GI, QL and HMF. MLF and HML work here by immediately giving a
principal type to $single\,id$ which involves generalization with a subtyping
bound. In contrast, FCIF makes no promises about principal typing --- which is
not feasible with dependent types --- and does no generalization, but it can
still infer a type for $id$ which can be later constrained to $IdTy$.
