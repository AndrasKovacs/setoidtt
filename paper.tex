...
\newcommand{\newConstancy}[6]{\mathsf{newConstancy}_{#1|#2,\,#3 : \Rec\,(#4\,#5)}\,#6}
...
\newcommand{\einsertpar}[5]{\mathsf{insert}_{#1|#2}\,#3\,#4\,#5}
...
and we extend it in Section \ref{sec:extending_elaboration}.
\begin{figure}[h]
\begin{alignat*}{4}
...
A$ in a surface program, we usually want to expand $id$ to $id\,\{\alpha\}$,
where $\alpha$ is a fresh metavariable.
\begin{mathpar}
\inferrule*{ins \in \{\true,\,\false\} \\ \Theta|\Gamma\vdash t : A}
           {\einsert{ins}{t}{A} \in \{(\Theta',\,\theta\,t',\,A')\,|\,
...
$A$ type, as follows. Here, we notate $\Theta$ and $\Gamma$ as subscripts but
later we will leave them implicit. The clauses of the following definition are
matched top-down.
\begin{alignat*}{3}
  &\einsertpar{\Theta}{\Gamma}{\false}{t}{A} && :\equiv\,\,&& (\Theta,\,\id,\,t,\,A)\\
  &\einsertpar{\Theta}{\Gamma}{\true\,}{t}{(\{x : A\}\to B)} && :\equiv&&
       \slet\,(\Theta',\,\theta,\,u) = \freshMeta{\Theta}{\Gamma}{A}\\
  & && &&\sin\,\einsertpar{\Theta'}{\Gamma[\theta']}{\true}{((t[\theta])\,\{u\})}{(B[\theta][x\mapsto u])}\\
  &\einsertpar{\Theta}{\Gamma}{\true\,}{t}{A} && :\equiv&& (\Theta,\,\id,\,t,\,A)
\end{alignat*}
...
inference, which toggles whether we perform implicit argument insertion on the
output. We explain the cases of the definition in order.
...
\begin{itemize}
  ...
  \item \emph{Polymorphic subtyping}. In some prior works, e.g.\ in
    ...
    implemented in any dependently typed setting. We also believe that it is an
    anti-feature in dependent settings, because elaboration of subtyping must
...
telescope. \LabTirName{curried-$\beta$} is still provable as a propositional
...
We use curried function space to close over record types in the scope. We
prepend the following clause to Definition \ref{def:closingtype}:
\[
...
\subsection{Handling Superfluous Implicit Functions}

Before we can move on to unification, checking and inference, we have to address
...
We use \emph{constancy constraints} to get rid of curried function types as
soon as we learn that they are non-dependent. These are constraints in the usual
sense in unification algorithms (e.g.\ as in \cite{abel2011higher} or
...
\begin{mathpar}
  \inferrule*[lab=metacon/constancy]
             {\Theta|\emptycon \vdash \alpha : \Delta \To \Tel \\ \Theta|\Gamma,\,x : \Rec\,(\alpha\,\ol{t}) \vdash B : \U}
             {\Theta,\,\mathsf{constancy}_{\Gamma,\,x : \Rec\,(\alpha\,\ol{t})}\,B\vdash}

  \inferrule*[lab=constancy-$\equiv$]
             {\Theta_0|\emptycon \vdash \alpha : \Delta \To \Tel \\ x \notin \FreeVars(B)}
             {\Theta_0,\,\mathsf{constancy}_{\Gamma_0,\,x : \Rec\,(\alpha\,\ol{t})}\,B,\,\Theta_1|\Gamma_1
               \vdash \alpha \equiv (\lambda\,\overline{\mathsf{vars}_{\Delta}}.\,\epsilon) : \Delta \To \Tel}

  \inferrule*[lab=metasub/constancy]
             {\Theta_1|\emptycon \vdash \alpha : \Delta \To \Tel \\ \theta : \Theta_0 \To \Theta_1 \\\\ (x \notin \FreeVars(B[\theta]))\,\,\,\text{implies}\,\,\,(\alpha[\theta] \equiv (\lambda\,\overline{\mathsf{vars}_{\Delta[\theta]}}.\,\epsilon))}
             {(\theta,\,\mathsf{solve}) : \Theta_0 \To (\Theta_1,\,\constancy_{\Gamma,\,x : \Rec\,(\alpha\,\ol{t})}\,B)}

  \inferrule*[lab=metasub/weaken-constancy]
             {\\}
             {\p : (\Theta,\,\constancy_{\Gamma,\,x : \Rec\,(\alpha\,\ol{t})}\,B) \To \Theta}
\end{mathpar}
\caption{Rules for constancy constraints}
\label{fig:constancy}
\end{figure}

In the rule \LabTirName{metacon/constancy} we specify extension of a
metacontext with a constraint. The metavariable $\alpha : \Gamma \To \Tel$
represents an unknown domain for a curried function type, and $B$ is the
codomain of the function.

The \LabTirName{constancy-$\equiv$} rule expresses that, assuming we have a
constancy constraint for $\alpha$ and $B$ in context, if $B$ does not depend on
the $x : \Rec\,(\alpha\,\ol{t})$ domain variable, then $\alpha$ is equal to the
constant function which returns the empty telescope. We use the abbreviation
$\lambda\,\overline{\mathsf{vars}_{\Gamma_0}}$ to refer to an iterated $\lambda$
which abstracts over all variables in $\Gamma_0$.
...
\Theta_0 \To \Theta_1$ to map into an additional constraint in the codomain if
$\theta$ forces the constraint to hold. In
\LabTirName{metasub/weaken-constancy}, we overload $\p$ for the weakening
substitution which drops a constraint.

\begin{definition}[Creating a new constraint]
We define this similarly to Definition \ref{def:freshmeta}, by simply
returning a weakening substitution.
\[
\newConstancy{\Theta}{\Gamma}{x}{\alpha}{\ol{t}}{B} : \equiv
             ((\Theta,\,\constancy_{\Gamma,\,x : \Rec\,(\alpha\,\ol{t})}\,B),\,\p)
\]
\end{definition}
...
augment the unification procedure with eager removal of solvable constraints.

After solving a meta $\beta$ during unification, which yields a unifying
$\theta$ substitution, we review all ($\constancy_{\Gamma,\,x :
  \Rec\,(\alpha\,\ol{t})}\,B$) constraints in the context, where $x$ occurs in
$B$ inside a $\ol{t}$ spine of some $\beta\,\ol{t}$ term. In other words, we
review constraints where the new meta solution might make a difference.
\begin{enumerate}
  \item If we have $x \in \FreeVars(B[\theta])$, where $x$ occurs rigidly in
    ...
    \LabTirName{metasub/constancy} to return a $\theta'$ substitution which also
    solves the constraint.
  \item If we have $x \notin \FreeVars(B[\theta])$, we solve $\alpha$ to be
    constantly $\epsilon$, thereby getting a $\theta'$ substitution. Since
    $\theta'$ forces the constrain to hold, we can again use
    \LabTirName{metasub/constancy} to solve the constraint. Then, since we have
    just solved $\alpha$, we recursively check for solvable constraints.
  \item
...
sound to solve the constraint when the occurrence is not necessarily
rigid. However, this way we could lose potential non-$\epsilon$ solutions of
$\alpha$.
...
non-empty, so we refine the domain telescope.
\begin{alignat*}{3}
& \unify_{\Theta_0|\Gamma}(\{x : \ol{A}\}\to B)\,(\{x : A'\}\to B') :\equiv \edo\\
& \quad (\Theta_1,\,\theta_1,\,\alpha\,\ol{t}) \leftarrow \freshMeta{\Theta_0}{\Gamma,\,x : A'}{\Tel}\\
& \quad (\Theta_2,\,\theta_2) \leftarrow
  \unify_{\Theta_1|\Gamma[\theta_1]}\,(A[\theta_1])\,((x : A'[\theta_1])\TCons \alpha\,\ol{t})\\
& \quad (\Theta_3,\,\theta_3) \leftarrow \newConstancy{\Theta_2}{\Gamma[\theta_1\circ\theta_2],\,y : A'[\theta_1\circ\theta_2]}{z}{(\alpha}{\ol{t})[\theta_2]}{(B[\theta_1\circ\theta_2][x\mapsto (y :: z)])}\\
& \unify_{\Theta_3|\Gamma[\theta_{123}]}\,(\{z : \ol{\alpha\,\ol{t}}
\end{alignat*}
