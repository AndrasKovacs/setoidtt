...
& \scriptstyle{6} && \qquad \einfert{true}{\true} = true : Bool\\
& \scriptstyle{8} && \qquad \mathsf{newConstancy}_{\emptycon,\,x : \Rec\,\alpha_1}\,Bool\\
& \scriptstyle{9} && \qquad\quad \unify\,\alpha_1\,\epsilon\\
& \scriptstyle{10} && \qquad \unify\,\alpha_0\,Bool\\
& \scriptstyle{11} && \quad = true\\
...
match on the shape of types, for example when we are inserting implicit
...
limitations, and how it compares to related works. We abbreviate our elaborator
as FCIF when comparing it to others.

\subsubsection{Related works}
MLF \cite{le2014mlf} is an extension of System F with polymorphic subtyping
bounds, which supports strong inference for first class polymorphism. HML
\cite{leijen2009flexible}) is a simplified variant of MLF. Since these systems
diverge markedly from (fragments of) Martin-Löf type theory and rely critically
on subtyping, it is unclear how they could be extended to dependent type
theories. HMF \cite{leijen2008hmf} is a relatively simple system with
...
application handling) jumps in complexity.
...
\cite{serrano2020a}. They both try to examine n-ary applications to find
...
of telescopes, but overall GI is also burdened by inessential complexity. QL
...
practically the most favorable so far, it is far from being elegant; it does
multiple checking passes, and the restriction of quick looking to neutral
applications is rather ad-hoc.
...
FCIF. The relative performance of FCIC here is easy to remember: it handles
...
FCIC does no let-generalization, the inferred types are not fully constrained
...
$\einfer{\lambda\,x.\,t}{ins}{\Theta}{\Gamma}$.
...
While in this benchmark FCIF matches QL, in other ways it is significantly
stronger. Some general observations on the relative strength of FCIF:
\begin{itemize}
  \item
        Most obviously, FCIF supports dependent type theory and prior solutions
        do not. It is also doubtful whether prior solutions scale to dependent
        types. In particular, QL's preprocessing step is not applicable, since in
        a dependently typed spine we cannot do anything with later arguments until
        previous arguments are actually elaborated, or else it has to be restricted
        to non-dependent spines.
  \item
        FCIF is global and does not consider neutral spines specially in any
        way. For an example for global inference, $\slet\,x : \_ =
        single\,id\,\sin\,$ $cons\,\{IdTy\}\,x\,nil$ works in FCIF but does not
        work in GI, QL and HMF. It also works in MLF and HML, which are also
        global in this sense. These work by immediately giving a principal type
        to $single\,id$ which includes a subtyping bound. In contrast, FCIF
        makes no promises about principal typing --- which is not feasible with
        dependent types --- but it can still infer a type for $id$ which can be
        later constrained to $IdTy$.
\end{itemize}

\emph{Limitations.} What FCIF definitely cannot do, is inferring an implicit
function type for a function argument. MLF is the only system which can do this,
in cases where the polymorphic argument is used only once. Consider the
following:
\[
  \slet\,f : IdTy \to Bool = \lambda\,\_.\,true\,\sin\,\lambda\,x.\,f\,x
  \]
FCIF will attempt to elaborate $f\,x$ to $f\,(\lambda\{A\}.\,x)$, but fails to
infer a type for $x$, because $A$ is not in the scope of $x$'s type. In the next
section we sketch an extension of FCIF which can handle this, and can also
handle $B2$ from Figure \ref{fig:benchmark}.
