...
possibly handle this example. We extend $\einsert$ with the following case. We
...
\ol{\alpha_0}\}\to \alpha_1\,y$, and we have to compute
...
assumption: metas like $\alpha_2$ are only solved with \emph{order-preserving
  embeddings}, so for $\alpha_2$ the four potential solutions are
$(\lambda\,x\,A.\,[])$, $(\lambda\,x\,A.\,(A :: []))$, $(\lambda\,x\,A.\,(x ::
[]))$ and $(\lambda\,x\,A.\,(x :: A :: []))$. The reason for only allowing such
solutions is that these cases can possibly yield solvable pattern unification
problems, while solutions containing arbitrary terms or non-linear variable
occurrences cannot. Out of these, $(\lambda\,x\,A.\,(x :: A :: []))$ and
...
would yield the unsolvable $\unify\,\alpha_1\,(A \to A)$ problem. Hence, there
is a unique order-preserving embedding solution in this case, which does yield
the expected elaboration output. However, note that this idea is far from being
fully fleshed out, and it could be subject of future research.
