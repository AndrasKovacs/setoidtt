...
-- pPos :: Parser Tm -> Parser Tm
-- pPos p = Src <$> (SrcPos <$> getSourcePos) <*> p
...
-- pLet = pPos $ do
--   symbol "let"
--   T3 x a t <- indent (pPos pIdent) $ \(Posed pos x) -> do
--     a <- optional (char ':' *> pTm)
--     t <- char '=' *> pTm
--     case a of
...
--     TEPostulate <$> pPos pIdent <*> (char ':' *> pTm)
...
-- pDefinition = nonIndented $ indent (pPos pIdent) $ \x@(Posed pos _) -> do
