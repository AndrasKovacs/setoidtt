...
sizedMap (I# size) f = \(Array (A.Array arr)) ->
    let go :: Int# -> SmallMutableArray# s Any -> Int# -> State# s -> State# s
        go i marr size s = case i <# size of
            1# -> case arr A.!# i of
              (# a #) -> case f $! fromUnlifted# (unsafeCoerce# a) of
                !a -> case toUnlifted# a of
                  a -> case writeSmallArray# marr i (unsafeCoerce# a) s of
...
fromList = \xs ->
  case length xs of
      I# size -> Array (A.Array (runRW# $ \s -> case newSmallArray# size undefElem s of
